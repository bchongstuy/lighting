...
def scanline_convert(polygons, i, screen, zbuffer, color):
    ...
    dx0 = (points[TOP][0] - points[BOT][0]) / \
        distance0 if distance0 != 0 else 0
    dz0 = (points[TOP][2] - points[BOT][2]) / \
        distance0 if distance0 != 0 else 0
    dx1 = (points[MID][0] - points[BOT][0]) / \
        distance1 if distance1 != 0 else 0
    dz1 = (points[MID][2] - points[BOT][2]) / \
        distance1 if distance1 != 0 else 0
    ...
    while y <= int(points[TOP][1]):
        if (not flip and y >= int(points[MID][1])):
            ...
            dx1 = (points[TOP][0] - points[MID][0]) / \
                distance2 if distance2 != 0 else 0
            dz1 = (points[TOP][2] - points[MID][2]) / \
                distance2 if distance2 != 0 else 0
            x1 = points[MID][0]
            z1 = points[MID][2]
...
def draw_polygons(polygons, screen, zbuffer, view, ambient, light, areflect, dreflect, sreflect):
    ...
    while point < len(polygons) - 2:
        ...
        if normal[2] > 0:
            ...
            color = get_lighting(normal, view, ambient,
                                 light, areflect, dreflect, sreflect)
